...
def darBaja(incoming_msg):
   ...
   #respInfoVeh = requests.get('http://192.168.1.194:4000/api/wsSao/getInfoVehic                                     ulo/1/'+ incoming_msg)
    respInfoVeh = requests.get('http://172.20.68.113:5001/api/wsSao/getInfoVehic                                     ulo/1/'+ incoming_msg)
    if respInfoVeh.status_code == 200:
            data = respInfoVeh.json()
            codRetorno = data['codRetorno']
            if codRetorno == '0001':

                respFech = requests.get('http://192.168.1.108:10544/apiAMTZonaAz                                     ul/Utiles/fechaSistema/*')
                ...
                respPagos = requests.get('http://172.20.68.113:5001/api/wsSao/ge                                     tPagos/1/'+codigoVehiculo)
                    ...
                    if codRetorno == '0001':
                        ...
                        for x in ordenes:
                            if x['DescripcionEstado'] == 'ACTIVO':
                                ...
                                #print('http://192.168.1.194:4000/api/wsSao/baja                                     AutomaticaOrdenPago/'+str(idOrden)+'/'+str(fecha))
                                respBaja = requests.get('http://172.20.68.113:50                                     01/api/wsSao/bajaAutomaticaOrdenPago/'+str(idOrden)+'/'+str(fecha))
