...
def darBaja(incoming_msg):
    ...
    respInfoVeh = requests.get(
        'http://172.20.68.113:5001/api/wsSao/getInfoVehiculo/1/' + incoming_msg)
    if respInfoVeh.status_code == 200:
        data = respInfoVeh.json()
        codRetorno = data['codRetorno']
        if codRetorno == '0001':
            respFech = requests.get('http://192.168.1.108:10544/apiAMTZonaAzul/Utiles/fechaSistema/*')
            fechaData = respFech.json()
            fecha = str(fechaData['retorno']).replace("/", "-")
            print('mod: ' + fecha)
            codigoVehiculo = data["retorno"]["CodigoVehiculo"]
            respPagos = requests.get('http://172.20.68.113:5001/api/wsSao/getPagos/1/'+codigoVehiculo)
                ...
                if codRetorno == '0001':
                    ...
                    for x in ordenes:
                        if x['DescripcionEstado'] == 'ACTIVO':
                            ...
                            respBaja = requests.get('http://172.20.68.113:5001/api/wsSao/bajaAutomaticaOrdenPago/'+str(idOrden)+'/'+str(fecha))
            ...
            body = 'No existe la placa consultada o el vehiculo se encuentrabloqueado consultar AXIS-SRI'
    else:
